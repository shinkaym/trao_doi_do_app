...
class Pagination extends HookConsumerWidget {
  ...
  final int currentTabIndex; // Thêm parameter để biết tab hiện tại
  ...
  const Pagination({
    ...
    required this.currentTabIndex, // Required parameter
  });
  ...
  // Helper methods để gọi đúng provider
  void _goToPreviousPage(WidgetRef ref) {
    if (currentTabIndex == 0) {
      ref.read(interestedPostsProvider.notifier).goToPreviousPage();
    } else {
      ref.read(postsWithInterestsProvider.notifier).goToPreviousPage();
    }
  }

  void _goToNextPage(WidgetRef ref) {
    if (currentTabIndex == 0) {
      ref.read(interestedPostsProvider.notifier).goToNextPage();
    } else {
      ref.read(postsWithInterestsProvider.notifier).goToNextPage();
    }
  }

  void _goToPage(int page, WidgetRef ref) {
    if (currentTabIndex == 0) {
      ref.read(interestedPostsProvider.notifier).goToPage(page);
    } else {
      ref.read(postsWithInterestsProvider.notifier).goToPage(page);
    }
  }
